...
// Find all --> Traz todos os médicos
router.get('/', ValidToken, (req, res) => {
  const sqlQuery = `SELECT * FROM Medicos 
                      WHERE d_e_l_e_t_ <> '*'`;
...
router.put('/', ValidToken, (req, res) => {
  Alteracao.execSQLQuery(req.body, res, 'Medicos');
});

// Inclusão de novo médico
router.post('/', ValidToken, (req, res) => {
  console.log(req.body);
  Cadastro.execSQLQuery(req.body, res, 'Medicos');
});

// Marca como deletado mantendo o histórico dos dados
router.delete('/', ValidToken, (req, res) => {
  Delete.execSQLQuery(req.body, res, 'Medicos');
});
